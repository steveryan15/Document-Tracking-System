<?php 
  $db = mysqli_connect('localhost', 'root', '', 'villa_dblgu');
   ...
   if (isset($_POST['accept'])) {
    ...
    $query = "UPDATE  tbl_documents SET Document_sent_status='$Document_sent_status', notify_user = 0, msg_stat=1, admin_notify=1 WHERE id='$id'";
...
// para ma butang na forwarded sa nag send saimong tanan history sa office niya
   ...
   $results = mysqli_query($db, $query);
    ...
    exit();
      ...
      }else if ($accept_refresh == 'view_all_users') {
       ?>
    <script>
    alert('Document has been accepted.');
...
if ($prob_qty == 'Decline') {
    ...
    $query = "UPDATE  tbl_documents SET Document_sent_status='$accept_and_closed_document', notify_user = 0, msg_stat=1, admin_notify=1 WHERE id='$id'";
       $results = mysqli_query($db, $query);
...
if (isset($_POST['closed'])) {
    ...
    $query = "UPDATE  tbl_documents SET Document_sent_status='$closed_document',  msg_stat=1, admin_notify=1 WHERE id='$id'";
       $results = mysqli_query($db, $query);
...
if ($prob_qty == 'Decline') {
    ...
    $query = "UPDATE  tbl_documents SET Froms = '$sender_decline', Status = '$receiver', personnel_para_history_log = '$sender', office_para_history_log = '$senders_office', Document_sent_status='$Document_sent_status', msg_stat = '$msg_stat', decline_message = '$decline_message' WHERE id='$id'";
