     ...
     if ($prob_qty == 'Received and Closed') {
            ...
            ?>
              ...
              alert('Document is already closed.');
                  window.location.href='admin_received_document_table.php?fail';
                  </script>
              <?php
                exit(); 
              }else {

   $query = "UPDATE  tbl_documents SET Document_type ='$doc_type', Content ='$doc_content', Froms ='$doc_from', Status ='$doc_status', Date_sent ='$Date_sent', Document_sent_status='$document_sent_status', personnel_para_history_log ='$personnel_para_history', office_para_history_log='$office_para_history', msg_stat ='$doc_msg', admin_notify ='$admin_msg' WHERE id = '$id' ";
   $results = mysqli_query($db, $query);
  ?>
    <script>
    alert('Document has been forwarded.');
        window.location.href='admin_received_document_table.php?success';
        </script>
    <?php
    exit(); 
    } 
